...
int main( )
{
    ...
    const double PropagationEndsS1 = -0.7E7;
    ...
    Eigen::VectorXd alpha1 = Eigen::VectorXd::LinSpaced(sampleSizeA1, -1E-999, -1E-998);//-5E-10, 0.0);
        ...
        for(int alpha2Counter = 0; alpha2Counter <= alpha2.size(); alpha2Counter++)
        {
            ...
            for(int betaCounter = 0; betaCounter <= beta.size(); betaCounter++)
            {
                ...
                for(int timeCounter = 0; timeCounter <= time.size(); timeCounter++)
                {
                    ...
                    xdot2 = sunDistance * rotationSpeed * xdot ;
                    ydot2 = sunDistance * rotationSpeed * ydot;
            ...
            for(int betaCounter = 0; betaCounter <= beta.size(); betaCounter++)
            {
                ...
                for(int timeCounter = 0; timeCounter <= time.size(); timeCounter++)
                {
                    ...
                    for(int moonCounter = 0; moonCounter <= moonAngle.size(); moonCounter++)
                    {
                        ...
                        y2 = MoonDistance * y;
                        xdot2 = MoonDistance * MoonRotationSpeed * xdot;
                        ydot2 = MoonDistance * MoonRotationSpeed * ydot;
    ...
    for(int fileCounter = 0; fileCounter <= FileNames.size()-1 ; fileCounter++)
    {
    ...
    std::string path;
    Eigen::MatrixXd S1segment;
    Eigen::MatrixXd S2segment;
    Eigen::MatrixXd S3segment;
    Eigen::MatrixXd solution;

    //Define the distance tolerance for matching the start- and endpoint of segments.
    double tolerance = 1E6;

    for(int S1Counter = 0; S1Counter <= S1Files.size()-2; S1Counter++)
        // For each of the Segment1 trajectories, find the passage through y=0 near the Lagrange point if there is any.
    {
        std::cerr<<"S1 nr " + boost::lexical_cast< std::string >( S1Counter ) <<std::endl;
        path = tudat_applications::getOutputPath( ) + "/" + S1Files.at(S1Counter);
        S1segment = input_output::readMatrixFromFile( path , ",", "#");

        stop = 0;
        for(int S1time = 0; S1time <= S1segment.row(0).size()-1; S1time++)
        {
            if(stop == 0)
            {
                if(S1segment(20,S1time) * S1segment(20,S1time+1) < 0)
                {
                    std::cerr<<"Segment 1 crosses x-axis"<<std::endl;
                    if(S1segment(19,S1time) > 1E5 ){
                        stop = 1;
                        S1endtime = S1time;
                        S1endX = S1segment(19,S1time);
                        std::cerr<<"Found suitable end location for S1" <<std::endl;
                    }
                }
            }
        }
        ...
        {
            for(int S2Counter = 0; S2Counter <= S2Files.size()-2; S2Counter++)
            ...
            {
                std::cerr<<"S2 nr " + boost::lexical_cast< std::string >( S2Counter ) + " of " +  boost::lexical_cast< std::string >(S2Files.size()) <<std::endl;
                path = tudat_applications::getOutputPath( ) + "/" + S2Files.at(S2Counter);
                S2segment = input_output::readMatrixFromFile( path, ",");

                stop = 0;
                int match12 = 0;
                for(int S2time = 0; S2time <= S2segment.row(0).size()-1; S2time++)
                {
                    if(stop == 0)
                    {
                        if(S2segment(20,S2time) * S2segment(20,S2time+1) < 0)
                        {
                            std::cerr<<"Segment 2 crosses x-axis"<<std::endl;
                            if(S2segment(19,S2time) > 1E5 )
                            {
                                stop = 1;
                                S2starttime = S2time;
                                S2startX = S2segment(19,S2time);
                                std::cerr<<"Found suitable start location for S2" <<std::endl;

                                if(std::abs(S1endX-S2startX) <= tolerance )
                                {
                                    match12 = 1;
                                    std::cerr<<"S1 end and S2 start match" <<std::endl;
                                }
                            }
                        }
                    }
                    ...
                    // if there is no match between segments 1 and 2, there is no use working on with this segment
                {
                    ...
                    for(int S2time = S2starttime; S2time<= S2segment.row(0).size()-1; S2time++)
                    {
                        if(stop == 0)
                        {
                            if(S2segment(19,S2time) * S2segment(19,S2time+1) < 0)
                            {
                                std::cerr<<"Segment 2 crosses y-axis"<<std::endl;
                                if(S2segment(20,S2time) > 0)
                                {
                                    stop = 1;
                                    S2endtime = S2time;
                                    S2endY = S2segment(20,S2time);
                                    std::cerr<<"Found suitable end location for S2" <<std::endl;
                                }
                            }
                        }
                    }
                    ...
                    {
                        for(int S3Counter = 0; S3Counter <= S3Files.size()-2; S3Counter++)
                        {
                            std::cerr<<"S3 nr " + boost::lexical_cast< std::string >( S3Counter ) <<std::endl;
                            path = tudat_applications::getOutputPath( ) + "/" + S3Files.at(S3Counter);
                            S3segment = input_output::readMatrixFromFile( path , ",");

                            stop = 0;
                            int match23 = 0;

                            for(int S3time = 0; S3time <= S3segment.row(0).size()-1; S3time++)
                            {
                                if(stop == 0)
                                {
                                    if(S3segment(19,S3time) * S3segment(19,S3time+1) < 0)
                                    {
                                        std::cerr<<"Segment 3 crosses y-axis"<<std::endl;
                                        if(S3segment(20,S3time) > 1E5 ){
                                            ...
                                            if(std::abs(S2endY-S3startY) <= tolerance )
                                            {
                                                match23 = 1;
                                                std::cerr<<"S2 end and S3 start match" <<std::endl;

                                                solution = S1segment.block(0,0,1,S1endtime);
                                                solution.block(1,0,2,S1endtime) =
                                                        S1segment.block(19,0,2,S1endtime);

                                                solution.block(0,S1endtime+1,1,S2endtime-S2starttime) =
                                                        S2segment.block(0,S2starttime,1,S2endtime-S2starttime);
                                                solution.block(1,S1endtime+1,2,S2endtime-S2starttime) =
                                                        S2segment.block(19,S2starttime,2,S2endtime-S2starttime);

                                                solution.block(0,S1endtime+S2endtime-S2starttime+1,1,S3segment.row(0).size()-S3starttime) =
                                                        S3segment.block(0,S3starttime,1,S3segment.row(0).size()-S3starttime) ;
                                                solution.block(1,S1endtime+S2endtime-S2starttime+1,2,S3segment.row(0).size()-S3starttime) =
                                                        S3segment.block(19,S3starttime,2,S3segment.row(0).size()-S3starttime);

                                                // Write solution to file for later access.
                                                std::ofstream("D:/FILES/Document/Thesis/Solutions/WP1_SOLUTION_S1 is " + boost::lexical_cast< std::string >( S1Counter )
                                                              + "_S2 is " + boost::lexical_cast< std::string >( S2Counter )
                                                              + "_S3 is " + boost::lexical_cast< std::string >( S3Counter )
                                                              + ".dat") << solution;

                                                std::cerr<<"COMPLETE ORBIT COMPLETE ORBIT COMPLETE ORBIT COMPLETE ORBIT" <<std::endl;
                                            }
                                        }
                                    }
                                }
